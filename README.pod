...
もしOpenCVを C</usr/local/> 下に設置した場合は、1. C</etc/ld.so.conf.d/>下に
ライブラリのパス（C</usr/local/lib>）を記述したファイルを設置した後、
C<ldconfig>などを実行する。（システムからの設定）2. または、実行時に
C<LD_LIBRARY_PATH>にC</usr/local/lib>を指定する、といった方法で
C</usr/local/lib>までライブラリのパスを通すこと。

=head2 How to make

インストールディレクトリに移動し、C<make>を実行する。

    cd pcolpicker
    make

実行プログラム C<pcolpicker> ができるので、これを任意の場所に設置する。
...
ADWAYS テスト環境内。
Cornic HSV Model（円錐型HSV） と Colmnar HSV Model（円柱型HSVモデル）で、２パターンを用意。
例からもわかるとおり、ものによっては、彩度0.6～0.7前後で取るのがいいかもしれない。
